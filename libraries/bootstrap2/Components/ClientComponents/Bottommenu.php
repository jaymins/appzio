...
trait Bottommenu
{
    ...
    public function getComponentBottommenu(array $parameters = array(), array $styles = array())
    {
        ...
        if (isset($this->model->bottom_menu_config['top_hilite'])) {
            $hilite = $this->color_background_color;
            $colorhelp = new \Color($background);
            $divider = $colorhelp->darken();
        } else {
            $colorhelp = new \Color($background);
        ...
        if (isset($column)) {
            if (isset($this->model->bottom_menu_config['divider_height'])) {
                $divider_height = $this->model->bottom_menu_config['divider_height'];
            } else {
                $divider_height = 2;
            }

            $row[] = $this->getComponentText('', array(), array('height' => $divider_height, 'background-color' => $divider));
        ...
        if (isset($this->model->bottom_menu_config['height'])) {
            $height = $this->model->bottom_menu_config['height'];
        }
    ...
    private function getItem($item, $count, $current, $hilite)
    {
        ...
        if (isset($this->model->bottom_menu_config['height'])) {
            $height = $this->model->bottom_menu_config['height'];
            $icon_height = round($height / 1.6, 0);
        } else {
            $icon_height = 25;
        ...
        /* special case where active menu item is indicated by line on the top rather than hiliting the whole backgound */
        if (isset($this->model->bottom_menu_config['top_hilite']) AND $this->model->bottom_menu_config['top_hilite']) {
            $active_hilite = $this->getComponentText('', [], [
                'height' => 2,
                'margin' => '0 0 -2 0',
                'background-color' => $this->model->bottom_menu_config['top_hilite'],
                'width' => $height]);
        }

        if ($item['action_config'] == $this->model->action_id AND $item['action'] == 'open-action' AND isset($item['icon_active']) AND $item['icon_active']) {
            if(isset($active_hilite)){ $row[] = $active_hilite; }
            $row[] = $this->getComponentImage($item['icon_active'], array(), array('height' => $icon_height, 'margin' => '8 0 5 0'));
            ...
            if ($item['icon']) $row[] = $this->getComponentImage($item['icon'], array(), array('height' => $icon_height, 'margin' => '8 0 5 0'));
        }
        ...
        $onclick->id = 'bottommenu';
        if(isset($this->model->bottom_menu_config['disable_cache'])){
            $onclick->disable_cache = 1;
        }
