...
class DiaryExport extends DiaryModel
{
   ...
   public static function ExportGame($gid){
        return DiaryModel::getGameDiary($gid);
	 ...
	 public function exportAction ($ai) {
        ...
        while($post = each($rows)){
           ...
           if($post['commentparentid'] > 0){
			   ...
			   $this->postsArr[$entryId]['postAuthorId']=$post['userid'];
