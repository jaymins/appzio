...
class DiaryPostModel extends DiaryModel
{
...
/**
	...
	public function relations()
	{
		...
		return array(
			'diary' => array(self::BELONGS_TO, 'DiaryModel', 'diary_id'),
		);
    }
    ...
    public static function getPosts($actionid){
        $sql = 'SELECT *,
                      ...
                      ae_ext_diary_post.msg AS `reply`,
                      ae_ext_diary.msg AS msg,
                      ae_ext_diary.date AS maindate,
                      (SELECT * FROM ae_ext_diary_post WHERE diary_id = entryid)
                    FROM ae_ext_diary
                    LEFT JOIN ae_ext_diary_post ON
                    ...
                    WHERE ae_ext_diary.playtask_id = :actionId
                    GROUP BY entryid
                    ORDER BY ae_ext_diary.date, ae_ext_diary_post.date DESC

                    ';

        // ORDER BY ae_game_play.user_id, ae_game_play_action.id, ae_ext_diary.date DESC
    ...
    public static function getGameDiary($gid){
        $sql = 'SELECT *,
                    ...
                    ORDER BY ae_diary_diary.date DESC
                    ';
