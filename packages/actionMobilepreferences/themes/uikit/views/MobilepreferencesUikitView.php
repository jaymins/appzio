...
class MobilepreferencesUikitView extends MobilepreferencesUikitController
{
    public $selectedState = array(
        'style' => 'uikituserpref_radio_selected_state',
        'variable_value' => '1',
        'allow_unselect' => 1,
        'animation' => 'fade'
    );
...
/*        $this->data->scroll[] = $this->getText('{#filters#}', array('style' => 'profile_field_label'));
        ...
        if($this->getConfigParam('allow_reset')){
            $parameters['border-color'] = $this->colors['button_color'];
            $parameters['color'] = $this->colors['button_color'];
            $parameters['margin'] = '20 80 0 80';
            $parameters['padding'] = '10 5 10 5';
            $parameters['border-radius'] = '20';
            $parameters['text-align'] = 'center';
            $parameters['onclick'][] = $this->getOnclick('id',false,'reset-matches');
            $parameters['onclick'][] = $this->getOnclick('go-home','save-data');
            $this->data->footer[] = $this->getText('{#reset_all_matches#}',$parameters);
            unset($parameters);
        }

        $parameters['background-color'] = $this->colors['button_color'];
        $parameters['color'] = $this->colors['button_text_color'];
        $parameters['margin'] = '15 80 20 80';
        $parameters['padding'] = '10 5 10 5';
        $parameters['border-radius'] = '20';
        $parameters['text-align'] = 'center';
        $parameters['onclick'][] = $this->getOnclick('id',false,'save-data');
        $parameters['onclick'][] = $this->getOnclick('go-home','save-data',['sync_open' => 1]);
        $this->data->footer[] = $this->getText('{#save#}',$parameters);
    ...
    {
        $this->data->scroll[] = $this->getText('{#birth_year#}', array('style' => 'uikituserpref_profile_field_label'));
    ...
    {
        $this->data->scroll[] = $this->getText('{#i_am_searching_for#}', array('style' => 'uikituserpref_profile_field_label_top'));
        ...
        $this->data->scroll[] = $this->getColumn(array(
            ...
            $this->getRow(array(
                ...
                $fieldRow[] = $this->getRow(array(
                    $this->getText('', array(
                        'style' => 'uikituserpref_radio_default_state',
                        'variable' => 'men',
                        'selected_state' => $this->selectedState,
                    ))
                ), array(
                    'style' => 'uikituserpref_column-right-aligned',
                ))
            ), array(
                'background-color' => '#ffffff',
                'height' => '45',
    ...
    {
        $this->data->scroll[] = $this->getText('{#distance#}', array('style' => 'uikituserpref_profile_field_label'));

        $distance = $this->getVariable('filter_distance');
        $distanceValue = empty($distance) ? 0 : $distance;

        $min_value = $this->getConfigParam('min_distance',0);
        $max_value = $this->getConfigParam('max_distance',50);

        $min_text = $min_value ? $min_value .' km' : '0 km';

        $this->data->scroll[] = $this->getColumn(array(
            $this->getHairline('#f3f3f3'),
            $this->getRow(array(
                $this->getText($min_text, array(
                    'width' => '33%',
                    'font-size' => '14'
                )),
                $this->getRow(array(
                ...
                $this->getText($max_value.' km', array(
                    'width' => '33%',
                    'text-align' => 'right',
                    'font-size' => '14'
                ))
    ...
    {
        $this->data->scroll[] = $this->getText('{#min_age#}', array('style' => 'uikituserpref_profile_field_label'));
        ...
        $this->data->scroll[] = $this->getColumn(array(
            ...
            $this->getRangeslider('', array_merge(
                ...
                $this->getSliderStyles(),
                array(
                    'right_track_color' => $this->color_topbar,
        ...
        $this->data->scroll[] = $this->getText('{#max_age#}', array('style' => 'uikituserpref_profile_field_label'));
...
/*        $this->data->scroll[] = $this->getColumn(array(
    ...
    public function getSliderStyles() {
        ...
        $slider_ball = $this->getImageFileName('slider_ball_blue.png');

        return array(
            'step' => '1',
            'left_track_color' => $this->color_topbar,
            'right_track_color' => '#bebebe',
            'thumb_color' => $this->color_topbar,
            'track_height' => '4px',
            'margin' => '12 20 12 20',
