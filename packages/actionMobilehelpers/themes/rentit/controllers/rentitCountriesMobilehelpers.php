...
class rentitCountriesMobilehelpers extends rentitMobilehelpersSubController {
                ...
                if ( !empty($wordlist) ) {
                    foreach($wordlist as $word){
                        $this->data->scroll[] = $this->getText($word, array(
                            ...
                            'onclick' => $this->closeAndSelect( $word )
                        ));
    ...
    public function displayAllCountries() {
        ...
        foreach ($countries as $code => $country) {
            $count++;
            $this->data->scroll[] = $this->getText($country, array(
                'style' => ( $count % 2 == 0 ? 'helper-search-result-even' : 'helper-search-result' ),
                'onclick' => $this->closeAndSelect( $country )
            ));
            $this->data->scroll[] = $this->getText('', array( 'style' => 'row-divider' ));
        }
    ...
    public function closeAndSelect( $value ) {
        ...
        $update_params = array(
            $this->getVariableId('country') => $value
        );
