...
class classifiedsMobileuserprofileSubController extends MobileuserprofileController
{
    ...
    public function showProfileView()
    {
        ...
        $name = isset($vars['firstname']) ? $vars['firstname'] : '{#unknown#}';
        $this->rewriteActionField('subject', $name);

        $this->getImageScroll($vars);
        $this->data->scroll[] = $this->getSpacer('5');

        $profile_comment = isset($vars['profile_comment']) ? $vars['profile_comment'] : '{#no_description#}';
