...
class PhotostreamModel extends CActiveRecord
{
...
/**
    ...
    public static function getAllPosts($actionid){
        $sql = 'SELECT *,
                      ...
                      ae_ext_photostream.parent_id AS commentparentid,
					  ae_game.user_id AS gameauthor,
                      usergroups_user.id AS userid,
					  usergroups_user.email AS useremail
                    FROM ae_ext_photostream
                    LEFT JOIN usergroups_user ON ae_ext_photostream.user_id = usergroups_user.id
                    ...
                    GROUP BY entryid
                    ORDER BY commentparentid DESC, ae_ext_photostream.date DESC
                    ';
    ...
    public function updateFlags(){
	 ...
	 public static function getAllPhotostreamActions($gameid){

        $obj = Aeactiontypes::model()->findByAttributes(array('shortname' => 'photostream'));
    ...
    public static function getUserVariables($gameid){
        ...
        $sql = 'SELECT *,
                    ...
                    LEFT JOIN ae_ext_photostream ON usergroups_user.id = ae_ext_photostream.user_id
                    WHERE ae_game_play.game_id = :gameId
                    AND ae_ext_photostream.id IS NOT NULL
