...
namespace packages\actionDitems\themes\clubs\Models;
...
class Model extends BootstrapModel
{
   ...
   note that if variable includes a string, it will overwrite it */
	...
	public function saveClubItem() {
        $item = new ItemModel();
        $item->name = $this->getSubmittedVariableByName('club_name');
        $item->game_id = $this->appid;
        $item->play_id = $this->playid;
        $item->status = 'active';
        $item->type = 'club';
        $item->place_id = $this->getSavedVariable('selected_football_club');
