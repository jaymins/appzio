...
trait HintedField {
    ...
    public function getHintedField($hint,$variablename,$type='text', array $parameters=array(),array $styles=array()) {
        ...
        if($this->model->getValidationError($variablename)) {
            $error[] = $this->getComponentText($hint .' ', array('style' => 'pref_hint','uppercase' => true));
            ...
            $out[] = $this->getComponentText($hint, array('style' => 'pref_hint','uppercase' => true));
        }
        ...
        $out[] = $this->getComponentText('',array('style' => 'mreg_divider'));

        switch($type){
            case 'text':
                $out[] = $this->getComponentFormFieldText($val,
                    array_merge($parameters,array('style' => 'pref_field')
                ));
                break;

            case 'textarea':
                $out[] = $this->getComponentFormFieldTextArea($val,
                    array_merge($parameters,array('style' => 'pref_field_textarea')
                    ));
                break;
