...
trait getPhotoField {
    ...
    public function getPhotoField(string $content, array $parameters=array(),array $styles=array()) {
        ...
        $parameters['style'] = 'mreg_pic';
        $parameters['variable'] = $this->model->getVariableId('profilepic');
        $parameters['onclick'] = $this->getOnclickImageUpload('profilepic',array(
            'max_dimensions' => '900',
            'sync_upload' => 1
        ));

        $obj[] = $this->getComponentImage($pic,$parameters);
