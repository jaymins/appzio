...
class Oauthconnector extends ActivationEngineAction {
...
/*      $var = AeplayVariable::getArrayOfPlayvariables($this->playid);
        ...
        if(isset($var['oauth_code'])){
            $result = file_get_contents($var['oauth_code']);
            print_r($result);
            die();
        } else {
            Appcaching::removeGlobalCache('oauth-authurl-'.$this->playid);
            Appcaching::removeGlobalCache('oauth-'.$this->playid);
            echo($this->playid.'There has been an error, please close this window and open it again');
            die();
        }*/


        if(isset($this->configdata->url)){
            $url = $this->configdata->url;
        } else {
            echo('Configuration error');die();
        }

        if(isset($this->configdata->client_id)){
            $client_id = $this->configdata->client_id;
        } else {
            echo('Configuration error');die();
        }

        if(isset($this->configdata->fields)){
            $fields = '&scope=' .$this->configdata->fields;
        } else {
            $fields = false;
        }
        ...
        $return = Controller::getDomain($this->gameid) .'/en/a/' .$this->token;
        AeplayVariable::updateWithName($this->playid,'oauth_return_url',$return,$this->gameid);

        $return = urlencode($return);
        $string = "location:$url?client_id=$client_id&redirect_uri=$return&response_type=code$fields";


        file_put_contents('dummy.txt',$string .serialize($_SESSION));
        //print_r($string);die();
