...
class MobileplacesWidget extends CWidget
{
    ...
    public function userinfo($cid = false){
        ...
        if ($cid) {
			...
			$diarysid = $this->comments_byid[$cid]['entryid'];
			} else {
			$show=0;	
			}
        } else {
        ...
        $output .= '<div class="diary_profile">';
        ...
        if(isset($this->configdata->profile_picture) AND $this->configdata->profile_picture > 0 AND isset($this->uservariables[$userid][$this->configdata->profile_picture]['variablevalue'])){
            ...
            if(stristr($pic,'.jpg') OR stristr($pic,'.png') OR stristr($pic,'.gif')){
                $output .= '<div class="diary_pic"><img src="';
                $ic = new ImagesController('bbs');
                $picurl = $ic->getImageUrl('120','120',$pic,'90','round','user');
                $output .= $picurl;
                $output .= '" width="60"></div>';
            } else { 
                $output .= '<div class="diary_pic"><img width="60" src="/images/milky/48/47.png"></div>';
           }
        ...
        if(isset($this->configdata->name) AND $this->configdata->name > 0 AND isset($this->uservariables[$userid][$this->configdata->name]['variablevalue'])){
            $output .= '<div class="date">' .$date .'</div><div class="diary_name">';
    ...
    public function buttons($entryid){
		...
		if (Yii::app()->user->id == $this->post['gameauthor']) {
			$queryStr='&ai='.$this->post['action_id'].'&ptid='.$this->post['playtask_id'].'&ui='.$this->post['userid'];
		}
        ...
        if( (($this->configdata->allow_delete==1) && (Yii::app()->user->id == $this->post['userid'])) || Yii::app()->user->id == $this->post['gameauthor']){
    ...
    public function msg(){
        ...
        if(isset($_REQUEST['a']) AND $_REQUEST['a'] == 'comment' AND isset($_REQUEST['id']) AND $_REQUEST['id'] == $this->post['entryid']){
            $output .= $this->commentBox($this->post['action_id'], $this->post['entryid'],$this->post['userid']);
		   ...
		   foreach ($this->comments[$parentid] as $answers) {
                  ...
                  $output .= $answers['msg'];
                    ...
                    if(isset($_REQUEST['a']) AND $_REQUEST['a'] == 'comment' AND isset($_REQUEST['id']) AND $_REQUEST['id'] == $answers['entryid']){
                          $output .= $this->commentBox($answers['action_id'], $answers['entryid'], $answers['userid']);
                          $output .= '<a href="' .$this->baseurl .'" class="cancel">{%cancel%}</a>';
                    } else {
    ...
    public function commentBox($actionid,$entryid,$userid){
        ...
        $formfields['elements'] = array(
			...
			'ui' => array('type' => 'hidden', 'value' => $userid)
        );
    ...
    public function newpostForm(){
	   ...
	   if (isset($this->configdata->commentboxtitle)) {$title= $this->configdata->commentboxtitle;} else { $title='';  }
      

        $output = Yii::app()->mustache->GetRender($this->templatepath .'diary-main',array(
                'form'=>$form,
                'uploadurl' => $uploadurl,
                'getimageurl' => $getimageurl,
                'brief' =>$brief,
                'commentboxtitle' => $title,
            ));
