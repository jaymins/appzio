...
trait getPhoneNumberField {
    ...
    public function getPhoneNumberField(string $country, $field, string $title, string $icon=''){
        ...
        if($test){
            $col[] = $this->getComponentImage($flag,array('style' => 'mplug_icon_field','onclick' => $this->getOnclickShowDiv('countries',$clickparams)));
        } else {
            $col[] = $this->getComponentImage('flag_questionmark.png',array('style' => 'mplug_icon_field','onclick' => $this->getOnclickShowDiv('countries',$clickparams)));
        }
