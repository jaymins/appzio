 ...
 public $VariableNameMap=array();
   ...
   public function exportAction($ai) {
        ...
        while($row = each($rows)){
			...
			$this->postsArr[$r['userId']]['userTimeZone']=$r['userTimeZone'];
			$this->postsArr[$r['userId']]['userPhone']=$r['userPhone'];
		    ...
		    foreach ($this->postsArr as $k=>$v){
			    ...
			    $this->postsArr[$k]['userTimeZone']=$v['userTimeZone'];
			    $this->postsArr[$k]['userPhone']=$v['userPhone'];
   ...
   public  function getUsersVars($ai) {
		...
		$rows = Yii::app()->db
          ...
          $sql = 'SELECT ae_game_play.user_id AS userId,
					   ...
					   usergroups_user.phone AS userPhone,
					   usergroups_user.timezone AS userTimeZone,
			 ...
			 ORDER BY ae_game_play.user_id ASC, ae_game_variable.id ASC
                    ';
